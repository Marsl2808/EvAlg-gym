...
class Neural_Network(object):
    ...
    def __init__(self, n_layer_nodes, weight_interval, weight_interval_output):
        ...
        for i in range(len(n_layer_nodes) - 1):
            self.weights.append( np.random.uniform(weight_interval[0], weight_interval[1], (n_layer_nodes[i], n_layer_nodes[i+1])))
            self.bias.append(np.random.uniform(low = weight_interval[0], high = weight_interval[1], size = n_layer_nodes[i+1]))
